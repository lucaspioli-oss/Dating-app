...
class FaceStorageService {
    static HASH_SIZE = 16; // Tamanho do hash (16x16)
    static SIMILARITY_THRESHOLD = 85; // % mínimo para considerar match
    static FACE_SIZE = 200; // Tamanho em pixels para salvar
    /**
    ...
    static async findCandidateAvatars(name, age, platform) {
        ...
        for (const doc of snapshot.docs) {
            ...
            if (!data.faceData?.faceHashes?.length)
                continue;
            ...
            if (age) {
                ...
                if (!ageMatch && possibleAges.length > 0)
                    continue;
            }
            candidates.push({
                id: doc.id,
