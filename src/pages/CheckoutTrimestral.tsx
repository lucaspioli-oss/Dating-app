...
const plan = {
  ...
  priceId: "price_1SgsmgAflPjpW4DO0oID3xaW",
  name: "Trimestral",
  price: "69,90",
  originalPrice: "89,70",
  period: "/trimestre",
  equivalent: "R$23,30/mes",
  discount: "22% OFF"
}

const API_URL = 'https://dating-app-production-ac43.up.railway.app';

export default function CheckoutTrimestral() {
  ...
  const checkAndCaptureLeadData = useCallback((currentName: string, currentEmail: string) => {
    ...
    if (currentName.trim().length >= 2 && emailRegex.test(currentEmail) && trimmedEmail !== lastCapturedEmailRef.current) {
      ...
      trackLeadCapture(currentName.trim(), currentEmail.trim(), 'checkout_trimestral')
  ...
  useEffect(() => {
    window.scrollTo(0, 0)
    trackViewContent('checkout_trimestral')
  }, [])
    ...
    <div className="min-h-screen bg-background text-foreground py-8 px-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="text-center" style={{ marginBottom: 24 }}>
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/20 text-primary text-xs font-bold mb-2 ml-2">
            {plan.discount}
          </div>
          ...
          <p className="text-sm text-primary mt-1">{plan.equivalent}</p>
          <p className="text-sm text-muted-foreground mt-2">Nao gostou? Devolvemos 100% em ate 7 dias.</p>
        </div>

        {/* Checkout Form */}
        <div className="bg-zinc-900/80 border border-primary/30" style={{ borderRadius: 20, padding: 32, marginBottom: 24 }}>
