...
const plan = {
  id: "anual",
  stripePlan: "yearly",
  priceId: "price_1SgsESAflPjpW4DO6Sd4z8n0",
  name: "Anual",
  price: "199,90",
  originalPrice: "358,80",
  period: "/ano",
  equivalent: "R$16,66/mes",
  discount: "44% OFF"
}
...
export default function CheckoutAnual() {
  ...
  const checkAndCaptureLeadData = useCallback((currentName: string, currentEmail: string) => {
    ...
    if (currentName.trim().length >= 2 && emailRegex.test(currentEmail) && trimmedEmail !== lastCapturedEmailRef.current) {
      ...
      trackLeadCapture(currentName.trim(), currentEmail.trim(), 'checkout_anual')
  ...
  useEffect(() => {
    window.scrollTo(0, 0)
    trackViewContent('checkout_anual')
  }, [])
    ...
    <div className="min-h-screen bg-background text-foreground py-8 px-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="text-center" style={{ marginBottom: 24 }}>
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary/20 text-secondary text-xs font-bold mb-2 ml-2">
            {plan.discount}
          </div>
          ...
          <p className="text-sm text-secondary mt-1">{plan.equivalent}</p>
          <p className="text-sm text-muted-foreground mt-2">Nao gostou? Devolvemos 100% em ate 7 dias.</p>
        </div>

        {/* Checkout Form */}
        <div className="bg-zinc-900/80 border border-secondary/30" style={{ borderRadius: 20, padding: 32, marginBottom: 24 }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 16 }}>
              ...
              <input
                ...
                className="w-full bg-zinc-800 border border-zinc-600 px-4 py-4 text-white placeholder-zinc-500 focus:outline-none focus:border-secondary transition-colors"
